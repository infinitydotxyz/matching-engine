...
contract MatchExecutor is IERC1271, IERC721Receiver, Ownable, Pausable {
  ...
  function isValidSignature(bytes32 message, bytes calldata signature) external view override returns (bytes4) {
    (bytes32 r, bytes32 s, bytes32 vBytes) = _decodePackedSig(signature, 32, 32, 1);
  ...
  function _broker(MatchExecutorTypes.ExternalFulfillments calldata externalFulfillments) internal {
    ...
    if (externalFulfillments.nftsToTransfer.length > 0) {
      for (uint256 i; i < externalFulfillments.nftsToTransfer.length; ) {
        bool isApproved = IERC721(externalFulfillments.nftsToTransfer[i].collection).isApprovedForAll(
          address(this),
          address(exchange)
        );

        if (!isApproved) {
          IERC721(externalFulfillments.nftsToTransfer[i].collection).setApprovalForAll(address(exchange), true);
        }
